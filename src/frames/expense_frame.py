...
class ExpenseFrame(ttk.Frame):
    ...
    def record_expense(self):
        ...
        if split_type == 'Equal':
            # Get the number of members selected
            num_members = sum(var.get() for chk, var in self.checkboxes)

            # Get the base amount to be paid by each member (rounded down to 2 decimal places)
            base_amount_per_member = floor((amount / num_members) * 100) / 100

            # Calculate the total amount that will be covered by the base amount
            covered_amount = base_amount_per_member * num_members

            # Calculate the remaining amount to be distributed
            remaining_amount = amount - covered_amount

            # Distribute the remaining amount (0.01 dollars) among members, starting from the first
            # until the remaining amount is exhausted
            split_details = {}
            for member, (chk, var) in zip(self.group.members.values(), self.checkboxes):
                if var.get():  # If the member is selected
                    # Add the base amount to split details
                    split_details[member] = base_amount_per_member

                    # If there is remaining amount, add 0.01 dollars to this member's share
                    if remaining_amount > 0:
                        split_details[member] += 0.01
                        remaining_amount -= 0.01  # Subtract the cent that has been allocated
        elif split_type == 'Exact Amounts':
            total_input_amount = 0
            split_details = {}
            for member, (chk, var) in zip(self.group.members.values(), self.checkboxes):
                if var.get():  # Only if the member is selected
                    entry = next((entry for member_entry, entry in self.member_entries if member_entry.id == member.id), None)
                    try:
